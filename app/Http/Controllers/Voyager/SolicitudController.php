...
class SolicitudController extends VoyagerBaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $solicitud->tipo_peticion = is_array($request->tipo_peticion)
            ? implode(', ', $request->tipo_peticion)
            : $request->tipo_peticion;


        // $solicitud->tipo_peticion = is_array($request->tipo_peticion)
        //     ? implode(', ', array_map('strval', $request->tipo_peticion))
        //     : (string) $request->tipo_peticion;
        ...
        $directory = "igac-solicitud/{$currentMonth}";
        ...
        try {
            ...
            $lastTicket = ArmorumappRadicado::where('codigo_radicado', 'like', "TICKET{$currentYear}%")
                ->orderBy('codigo_radicado', 'desc')
                ->first();

            // Extraer el número del último código generado
            if ($lastTicket) {
                $lastNumber = (int)substr($lastTicket->codigo_radicado, 10);
            } else {
                $lastNumber = 0;
            }

            // Incrementar el número y generar el nuevo código
            $newNumber = str_pad($lastNumber + 1, 4, '0', STR_PAD_LEFT);
            $codigoTicket = "TICKET{$currentYear}" . $newNumber;

            // Cantidad inicial de días hábiles
            $diasHabiles = 15;

            // Crear el nuevo ticket
            $ticket = new ArmorumappRadicado();
            $ticket->codigo_radicado = $codigoTicket;
            $ticket->usuario = $user->id;
            // $ticket->tipo_peticion = $request->tipo_peticion;
            $ticket->tipo_peticion = is_array($request->tipo_peticion)
                ? implode(', ', $request->tipo_peticion)
                : $request->tipo_peticion;

            $ticket->estado = 'Radicado';
            $ticket->documento_tercero = $user->username;
            $ticket->fecha_respuesta = "{$diasHabiles} Días Hábiles"; // Guardamos como "15 Días Hábiles"
            $ticket->save();

            // Asociar el ticket a la solicitud
            $data->codigo_radicado = $codigoTicket;
            $data->estado = $ticket->id;
            $data->save();

            // Notificar al usuario y al administrador
            $adminEmail = Voyager::setting('admin.correo', 'ticsistemasptoboy@gmail.com');
        ...
        if (!$request->has('_tagging')) {
            if (auth()->user()->can('browse', $data)) {
                $redirect = redirect()->route("voyager.igac-radicado.index"); // Redirección al slug correcto
            } else {
