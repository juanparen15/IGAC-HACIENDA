                    ...
                    <div class="panel panel-bordered">
                        ...
                        <div class="panel-body">
                            ...
                            @php
                                ...
                                $selected_tipo_peticion = isset($dataTypeContent->tipo_peticion)
                                    ? json_decode($dataTypeContent->tipo_peticion)
                                    : [];
                            @endphp

                            <div class="form-group">
                                <h5 for="tipo_peticion">{{ __('Tipo de Reclamo') }}</h5>
                                <select multiple class="form-control select2" id="tipo_peticion" name="tipo_peticion[]">
                                    @foreach ($tipoPeticiones as $tipo_peticion)
                                        <option value="{{ $tipo_peticion->tipo_peticion }}"
                                            {{ in_array($tipo_peticion->id, $selected_tipo_peticion) ? 'selected' : '' }}>
                            ...
                            {{-- @php
                                // $tipoPeticiones = App\Models\TipoPeticion::all();
                                $tipoPeticiones = Voyager::tipo_peticion();
                                $selected_tipo_peticion = $dataTypeContent->tipo_peticion ?? [];
                            @endphp

                            <div class="form-group">
                                <label for="tipo_peticion">{{ __('Tipo de Petición') }}</label>
                                <select class="form-control select2" id="tipo_peticion" name="tipoPeticiones[]" multiple>
                                    @foreach ($tipoPeticiones as $tipoPeticion)
                                        <option value="{{ $tipoPeticion->id }}"
                                            {{ in_array($tipoPeticion->id, $selected_tipo_peticion) ? 'selected' : '' }}>
                            ...
                            {{-- <div class="form-group">
                                ...
                                <select class="form-control select2" id="tipo_peticion" name="tipo_peticion[]" multiple>
                                    @foreach ($tipoPeticiones as $tipo_peticion)
                                        <option value="{{ $tipo_peticion->tipo_peticion }}"
                                            @if (!empty($selected_tipo_peticion) && in_array($tipo_peticion->tipo_peticion, (array) $selected_tipo_peticion)) selected @endif>
                                            {{ $tipo_peticion->tipo_peticion }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group" id="otro_archivo_option">
                                <h5 for="otro_archivo">{{ __('Subir archivo (opcional)') }}</h5>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control richTextBox" name="mensaje">{{ old('mensaje', $dataTypeContent->mensaje ?? '') }}</textarea>
                            </div>
